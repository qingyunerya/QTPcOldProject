    ...
    ui(new Ui::MainWindow)
{
    ...
    connect(uSocket, SIGNAL(readyRead()), this, SLOT(udp_receive()));
...
void MainWindow::on_file_trasport_clicked()
{
    client_widget=new widget();
    client_widget->show();
}

void MainWindow::on_file_trasport_sever_2_clicked()
{
    sever_widget=new widget2();
    sever_widget->show();
}

void MainWindow::on_udp_sever_clicked()
{
    QString address=ui->lineEdit_ca->text();
    QString sport=ui->lineEdit_cp->text();
    if(QString(address).isEmpty()&& QString(sport).isEmpty()){ui->lineEdit_ca->setText("192.168.31.221");address="192.168.31.221"; ui->lineEdit_cp->setText("8080");sport="8080";}
    quint16 port=quint16(sport.toUInt());
    uSocket->bind(QHostAddress(address), port);
    debug("udp等待连接");
}
void MainWindow::udp_receive()
{
    ...
    ui->textEdit_sa->setText("这是一个udp连接"+str);
    }
}
